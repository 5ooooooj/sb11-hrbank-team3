...
핵심 인사 데이터를 중앙에서 안전하게 관리하고, Spring Batch 기반의 자동화된 주기적 백업, 리포트 생성으로 HR 담당자가 반복업무 없이 데이터를 신뢰할
수 있는 EMS
...
- **이윤선**: 데이터 백업 관리(Batch) 및 Railway 배포 환경 구성
...
| `bugfix/{이슈번호}`         | 기능 개발 중 발생한 버그를 수정하고 통합하는 브랜치                       |
